      ...
      <ul className="grid h-full grid-cols-2 items-stretch gap-[2rem]">
        {membershipsCard.map((member) => (
          <li
            ...
            key={member.level}
          >
            <h4 className="font-rubik text-[3.2rem] font-semibold text-grey-200">
              Level {member.level}
            </h4>
            <p className="pb-[1.8rem] text-lg font-medium leading-[3rem] tracking-[0.02em] text-grey-750">
              {member.title}
            </p>
            <ul className="flex flex-wrap items-center gap-x-[1rem] pb-[2.4rem]">
              {member.service.map((item, index) => (
                <li
                  className="flex items-center gap-4 text-md font-semibold leading-[3rem] tracking-[0.02em] text-grey-750"
                  key={index}
                >
